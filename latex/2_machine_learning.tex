...
\end{figure}
\subsection{本體建立}
本體及感測器是進行專案的重要基礎，各項參數要設定得宜才能得到正確的結果。\\
以下介紹幾種較為常見的參數設定及其特性：
\begin{itemize}
%=----------Body is respondable----------=%
...
%=----------Body is dynamic----------=%
SigmoidPrime Function(圖.\ref{SigmoidFunction}，紅色的)是從Sigmoid Function(圖.\ref{SigmoidFunction} )具有動態屬性的物體會在重力影響下墜落或在其它外力、力矩作用下發生運動狀態的改變。而靜態物體則會在場景中靜止不動，如地面，或跟隨其父節點運動（或跟隨其父節點在場景層次結構中的移動）。
\begin{figure}[hbt!]
\begin{center}
